	...
	namespace estimators
	{
		namespace lag_prediction
		{
			...
			ns_consts::EnmReturnStatus outputLaTeXHeader(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				// prepend subsection 
				// -------------------------------------------------------------------------- //
				std::wstring	strSubsection638 = std::wstring();
				std::wstring	strSubsectionTitle638 = std::wstring(L"Lag Prediction Estimate (NIST SP 800-90B Section 6.3.8)");
				std::wstring	strLabel = std::wstring();
				switch (io_refData.bits_per_sample)
				{
				case 1:
					strLabel = std::wstring(L"sec:Binary638");
					break;
				default:
					strLabel = std::wstring(L"sec:NonBinary638");
					break;
				}
				ns_spt::getLaTeXSubsection(strSubsection638, strSubsectionTitle638, strLabel);
				// -------------------------------------------------------------------------- //
				//
				// -------------------------------------------------------------------------- //
				(*io_refData.p_ssLaTeXFragment) << strSubsection638;
			...
			ns_consts::EnmReturnStatus outputSupplementalInfoLaTeX(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				{
				case 1:
					strLabel = std::wstring(L"sec:Binary638-traceability");
					break;
				default:
					strLabel = std::wstring(L"sec:NonBinary638-traceability");
					break;
				}
