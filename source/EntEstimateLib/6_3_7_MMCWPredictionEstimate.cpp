	...
	namespace estimators
	{
		namespace mmcw_prediction
		{
			...
			ns_consts::EnmReturnStatus outputLaTeXHeader(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				// prepend subsection 
				// -------------------------------------------------------------------------- //
				std::wstring	strSubsection637 = std::wstring();
				std::wstring	strSubsectionTitle637 = std::wstring(L"Multi Most Common in Window Prediction Estimate (NIST SP 800-90B Section 6.3.7)");
				std::wstring	strLabel = std::wstring();
				switch (io_refData.bits_per_sample)
				{
				case 1:
					strLabel = std::wstring(L"sec:Binary637");
					break;
				default:
					strLabel = std::wstring(L"sec:NonBinary637");
					break;
				}
				ns_spt::getLaTeXSubsection(strSubsection637, strSubsectionTitle637, strLabel);
				// -------------------------------------------------------------------------- //
				//
				// -------------------------------------------------------------------------- //
				(*io_refData.p_ssLaTeXFragment) << strSubsection637;
			...
			ns_consts::EnmReturnStatus outputLaTeXFooter(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				(*io_refData.p_ssLaTeXFragment) << L"coordinates {(" << io_refData.t_6_3_7.r - 1 << L", " << io_refData.t_6_3_7.occurrences_at_longest_run << L") ";
				(*io_refData.p_ssLaTeXFragment) << L"(" << io_refData.t_6_3_7.r - 1 << L", "<< io_refData.t_6_3_7.occurrences_at_longest_run << L")}" << L"\n";
				(*io_refData.p_ssLaTeXFragment) << L"node[above left] at (axis cs:" << io_refData.t_6_3_7.r - 1 << L", ";
				(*io_refData.p_ssLaTeXFragment) << io_refData.t_6_3_7.occurrences_at_longest_run << L") ";
				(*io_refData.p_ssLaTeXFragment) << L"{\\shortstack{$r - 1$ = ";
				(*io_refData.p_ssLaTeXFragment) << io_refData.t_6_3_7.r - 1 << L" " << L"\n";
			...
			ns_consts::EnmReturnStatus outputSupplementalInfoLaTeX(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				{
				case 1:
					strLabel = std::wstring(L"sec:Binary637-traceability");
					break;
				default:
					strLabel = std::wstring(L"sec:NonBinary637-traceability");
					break;
				}
				...
				(*io_refData.p_ssLaTeXFragment) << L"\\caption{Supplemental information for traceability (NIST SP 800-90B Section 6.3.7)}" << L"\n";
				...
				const ns_dt::t_data_for_estimator& i_refData)
			{
				...
				const int		N = i_refData.p_bzInputS->length(blitz::firstDim) - i_refData.t_6_3_7.w[0];
				...
				for (int i = i_refData.t_6_3_7.w[0] + 1; i <= i_refData.p_bzInputS->length(blitz::firstDim); ++i)
				{
					...
					for (int j = 0; j < 4; ++j)
					{
						if (i_refData.t_6_3_7.w[j] < i)
						{
							...
							const ns_consts::EnmReturnStatus	stsGetFreq = histArray.getFrequent(bin_frequent, i - i_refData.t_6_3_7.w[j] - 1, i - 2, i_refData);
							if (ns_consts::EnmReturnStatus::Success != stsGetFreq)
							{
								return sts = stsGetFreq;
							}

							io_ref_bz_frequent(j) = { false, static_cast<ns_dt::octet>(bin_frequent.idx) };
						}
						else
						{
					...
					// -------------------------------------------------------------------------- //
					const ns_dt::NullableT<ns_dt::octet>	prediction = io_ref_bz_frequent(io_ref_winner - 1);
					// -------------------------------------------------------------------------- //
					...
					for (int j = 0; j < 4; ++j)
					{
						...
						if (false == io_ref_bz_frequent(j).bIsValueNull)
						{
							if (io_ref_bz_frequent(j).value == (*(i_refData.p_bzInputS))(i - 1))
							{
								...
								// -------------------------------------------------------------------------- //
								// 2. If scoreboard_{j} >= scoreboard_{winner}, let winner = j
								// -------------------------------------------------------------------------- //
			...
			ns_consts::EnmReturnStatus check_args_for_estimate(const ns_dt::t_data_for_estimator& i_refData)
			{
				...
				if (static_cast<int>(i_refData.L) < i_refData.t_6_3_7.w[3])
				{
					return sts;
				}
			...
			ns_consts::EnmReturnStatus estimate(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				io_refData.t_6_3_7.occurrences_at_longest_run = 0;
				sts = step6(io_refData.t_6_3_7.r, io_refData.t_6_3_7.p_local, bz_correct, N, 
					io_refData.t_6_3_7.occurrences_at_longest_run, 
