	...
	namespace estimators
	{
		namespace mmcw_prediction
		{
			...
			ns_consts::EnmReturnStatus outputLaTeXFooter(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				(*io_refData.p_ssLaTeXFragment) << L"coordinates {(" << io_refData.t_6_3_7.r - 1 << L", 1.05) (" << io_refData.t_6_3_7.r - 1 << L", 1.05)}" << std::endl;
				(*io_refData.p_ssLaTeXFragment) << L"node[above left] at (axis cs:" << io_refData.t_6_3_7.r - 1 << L", 1.0) {\\shortstack{$r - 1$ = ";
			...
			ns_consts::EnmReturnStatus estimate(ns_dt::t_data_for_estimator& io_refData)
			{
				...
				sts = step6(io_refData.t_6_3_7.r, io_refData.t_6_3_7.p_local, bz_correct, N, ssFragmentForLaTeX);
				if (ns_consts::EnmReturnStatus::Success != sts)
				{
					return sts;
				}
