...
\Statex \tikzmk{A}
		\If {$0 < \eta_{\textrm{prev}}$}
			\State $\theta_{\textrm{work}}[\eta_{\textrm{prev}}] \gets \lambda$
		\EndIf
		\State $\lambda \gets 1$
\tikzmk{B}
\boxit{hlblue}
...
%%%%%			\If {$\theta_{\textrm{master}}[1] < \lambda$}
%%%%%				\State $\theta_{\textrm{master}}[1] \gets \lambda$
...
\boxit{Pomme!40}
	\Else
\Statex \tikzmk{A}
		\State \textbf{AccumulateLambda}($\mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}, \lambda$)
%%%%%		\State $\theta_{\textrm{work}}[\eta_{\textrm{prev}}] \gets \lambda$
%%%%%
%%%%%		\For{$\eta \gets (\eta_{\textrm{prev}} - 1) \Downto \textrm{\textbf{max}}(\eta_{\textrm{current}}, 1) $}
%%%%%			\State $\theta_{\textrm{work}}[\eta] \gets \theta_{\textrm{work}}[\eta] + \theta_{\textrm{work}}[\eta + 1] $
%%%%%			\Statex \Comment {sum up $\lambda$ and update the array $\theta$}
%%%%%		\EndFor
		
		\State \textbf{UpdateLambda}($\lambda, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{current}}$)
%%%%%		\If {$\eta_{\textrm{current}} = 0$}
%%%%%			\State $\lambda \gets 1$
%%%%%		\Else
%%%%%			\State $\lambda \gets (\theta_{\textrm{work}}[\eta_{\textrm{current}}] + 1)$
%%%%%		\EndIf
%%%%%		\If {$\eta_{\textrm{current}} = 0$}
%%%%%		\Else
%%%%%			\State $\theta_{\textrm{work}}[\eta_{\textrm{current}}] \gets \theta_{\textrm{work}}[\eta_{\textrm{current}}] + \theta_{\textrm{work}}[\eta_{\textrm{current}} + 1] $
%%%%%		\EndIf

		\State \textbf{UpdateThetaMaster}($\mib{\theta}_{\textrm{master}}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$)
%%%%%		\For{$\eta \gets \eta_{\textrm{prev}} \Downto (\eta_{\textrm{current}} + 1)$}
%%%%%			\If {$\theta_{\textrm{master}}[\eta] < \theta_{\textrm{work}}[\eta]$}
%%%%%				\State $\theta_{\textrm{master}}[\eta] \gets \theta_{\textrm{work}}[\eta]$
%%%%%				\Statex \Comment {update $\eta$-tuple count}
%%%%%			\EndIf
%%%%%		\EndFor

		\State \textbf{ResetThetaWork}($\mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$)
...
%%%%%
%%%%%
\State \textbf{AccumulateLambda}($\mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, 0, \lambda$)
%%%%%
%%%%%
%%%%%
\State \textbf{UpdateThetaMaster}($\mib{\theta}_{\textrm{master}}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, 0$)
%%%%%
%%%%%
%%%%%
\State $t \gets \textrm{\textbf{max}} \left( \left\{ \eta \in \{1, \dots, \eta_{\textrm{max}}\} \mid \theta_{\textrm{master}}[\eta] \geq 34 \right\} \right)$
\Statex \Comment {Find the largest $t$ such that the number of occurrences of the most common $t$-tuple in $\mib{S}$ is at least 35. This can be rewritten as shown above using $\mib{\theta}_{\textrm{master}}$.}
%%%%%
%%%%%
%%%%%
\State return \textbf{CalcQ}($\mib{\theta}_{\textrm{master}}, t$)
%%%%%
%%%%%
%%%%%
%%%%%\State return Q
...
%%%%%
%%%%%
		\State \textbf{UpdateNumerator}($\mib{\sigma}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$)
%%%%%
%%%%%
%%%%%
		\State \textbf{UpdateLambda}($\lambda, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{current}}$)
...
%%%%%
%%%%%
\State \textbf{UpdateNumerator}($\mib{\sigma}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, 0$)
%%%%%
%%%%%
%%%%%
\State \textbf{UpdateThetaMaster}($\mib{\theta}_{\textrm{master}}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, 0$)
...
%%%%%
%%%%%
\State $u \gets \textrm{\textbf{min}} \left( \left\{ \eta \in \{1, \dots, \nu\} \mid \theta_{\textrm{master}}[\eta] < 34 \right\} \right)$
\Statex \Comment {Find the smallest $u$ such that the number of occurrences of the most common $u$-tuple in $\mib{S}$ is less than 35. This can be rewritten as shown above using $\mib{\theta}_{\textrm{master}}$.}
%%%%%
%%%%%
%%%%%
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{AccumulateLambda}{$\mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}, \lambda$}
		\State $\theta_{\textrm{work}}[\eta_{\textrm{prev}}] \gets \lambda$
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{UpdateLambda}{$\lambda, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{current}}$}
		\If {$\eta_{\textrm{current}} = 0$}
			\State $\lambda \gets 1$
		\Else
			\State $\lambda \gets (\theta_{\textrm{work}}[\eta_{\textrm{current}}] + 1)$
...
\begin{algorithm}
  \caption{UpdateThetaMaster}\label{alg:UpdateThetaMaster}
  \begin{algorithmic}[1]
    \Function{UpdateThetaMaster}{$\mib{\theta}_{\textrm{master}}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$}
		\For{$\eta \gets \eta_{\textrm{prev}} \Downto (\eta_{\textrm{current}} + 1)$}
			\If {$\theta_{\textrm{master}}[\eta] < \theta_{\textrm{work}}[\eta]$}
				\State $\theta_{\textrm{master}}[\eta] \gets \theta_{\textrm{work}}[\eta]$
				\Statex \Comment {update $\eta$-tuple count}
...
\begin{algorithm}
  \caption{ResetThetaWork}\label{alg:ResetTheta}
  \begin{algorithmic}[1]
    \Function{ResetThetaWork}{$\mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$}
		\For{$\eta \gets \eta_{\textrm{prev}} \Downto \textrm{\textbf{max}}(\eta_{\textrm{current}}, 1)$}
			\State $\theta_{\textrm{work}}[\eta] \gets 0$
			\Statex \Comment {reset to zero for future computation}
		\EndFor
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{CalcQ}{$\mib{\theta}_{\textrm{master}}, t$}
%%%%%		\For{$\eta \gets 1, (t + 1)$}
%%%%%			\State $Q[\eta] \gets 0$
%%%%%		\EndFor
		\For{$\eta \gets 1, t$}
			\State $Q[\eta] \gets \theta_{\textrm{master}}[\eta] + 1$
		\EndFor
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{UpdateNumerator}{$\mib{\sigma}, \mib{\theta}_{\textrm{work}}, \eta_{\textrm{prev}}, \eta_{\textrm{current}}$}
		\For{$\eta \gets \eta_{\textrm{prev}} \Downto (\eta_{\textrm{current}} + 1) $}
			\Statex \Comment {Here, $\theta_{\textrm{work}}[\eta_{\textrm{current}}]$ is subject to change, therefore, updating $\sigma[\eta]$ where $\eta > \eta_{\textrm{current}}$ makes sense.}
			\If {$\theta_{\textrm{work}}[\eta] + 1 \ge 2$}
				\State $\displaystyle \sigma[\eta] \gets \sigma[\eta] + \binom{\theta_{\textrm{work}}[\eta] + 1}{2}$
			\EndIf
		\EndFor
