...
\begin{tikzpicture}
	...
	\draw (4, 4.2) node {$\tfrac{1}{2} \ln \frac{t_{01} t_{10}}{t_{11}^{2}} = \ln \frac{t_{00}}{t_{11}}$} ;
	%%%%%
...
\begin{enumerate}[{d)}-i)]
	...
	\end{align}
\end{enumerate}
Note here that it is used that the property $\lambda$ being even.

For eqs. (\ref{expression:neg1}), (\ref{expression:neg2}), (\ref{expression:neg3}), and (\ref{expression:neg4}), readers may think that the dominant sequence is inconsistent with the first sample value of the sequence. 
...
\end{bmatrix}
\end{align}
%この遷移確率行列に基づいて式(\ref{eq:fb})及び式(\ref{eq:fc})に限って計算を進めれば, $t_{00}$のべきを大きくした方が確率が大きいので, $00 \cdots 00$という系列が優位になり, 最大値を与える解は式(\ref{expression:nonneg1})と式(\ref{expression:neg3})に絞られる. 
%さらに, $p_{0}$自体は$p_{1}$よりも小さいため, ($t_{10} = 1$という事情にも依存するが, )式(\ref{expression:nonneg1})よりも式(\ref{expression:neg3})の方が大きい.
...
The solutions that give the maximum value are limited to those in eqs. (\ref{expression:nonneg1}) and (\ref{expression:neg3}). Furthermore, since $p_{0}$ itself is smaller than $p_{1}$, eq. (\ref{expression:neg3}) is larger than eq. (\ref{expression:nonneg1}) (although it depends on the circumstance that $t_{10} = 1$).
