...
\begin{tikzpicture}
	...
	\draw (-0.5, -1) node {$\frac{\lambda - 2}{2}$} ;
\end{tikzpicture}

In order to calculate the extreme values of $f_{a}, f_{b}, f_{c}$, and $f_{d}$, we first consider the partial derivatives of $f_{a}(\mib{T}, \mu, \nu)$ with respect to $\mu$, and $\nu$.
...
Taking into accout eq. (\ref{eq:nondiagonalproduct}) and eq. (\ref{eq:diagonal}), and the property that the value $f_{a}(\mib{T}, \mu, \nu)$ is non-negative, 
the following rough phase diagram would be obtained, by taking the horizontal axis as $\ln \frac{t_{00}}{t_{11}}$, and the vertical axis $\tfrac{1}{2}\ln \frac{t_{01} t_{10}}{t_{11}^{2}}$.
